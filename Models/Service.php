...
class Service extends Model
{
	public function action($action, $output = false) {
		...
		// $ansi->appendString($ssh->read($node->ssh_login . '@'.$node->ssh_host.':~$'));
		...
		if($output){
			foreach (json_decode($template->data, true)["_panel"] as $item){
				if($item['action'] == $action && $item['substr_from']) {
					$log = substr($log, strpos($log,$item['substr_from'])+strlen($item['substr_from']));
				}
			}
		}
	    ...
	    return json_encode(["status" => "success", "message" => "Action performed successfully.", "data" => ($output || auth()->user()->admin ? $log : "")]);
	}
